...
export class DataImportService implements IDataImportService {
  ...
  async importFromCsv(filePath: string): Promise<void> {
    ...
    for (const row of rows) {
      ...
      // Створюємо або отримуємо готель
      const entityType = row.entityType;
      const entityName = row.entityName;

      if (entityType === 'hotel' && !hotelsMap.has(entityName)) {
      ...
      // Зберігаємо дані для відгуку (створимо після збереження користувачів та готелів)
      if (entityType === 'hotel') {
        reviews.push({
